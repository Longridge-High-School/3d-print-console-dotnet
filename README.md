# 3D Print Console .NET

**[3D Print Console](https://github.com/Longridge-High-School/3d-print-console) running on .NET**

## Features

- Password-protected admin user interface. No more having to change files via SFTP!
- Full compatibility with 3D Print Console. It still uses all the original code, and all the 3D Print Console logic runs in your browser, just like it always has.